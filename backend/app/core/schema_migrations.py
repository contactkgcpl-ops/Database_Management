...
def migrate_orders_schema(db: Session) -> None:
    inspector = inspect(engine)
    tables = set(inspector.get_table_names())
    dialect = engine.dialect.name
    ...
    if "orders" not in tables:
        if dialect == "mysql":
            ...
            db.execute(text("CREATE UNIQUE INDEX ix_orders_order_number ON orders (order_number)"))
        db.commit()

    if "orders" in tables:
        orders_columns = {column["name"] for column in inspector.get_columns("orders")}
        if "company_name" not in orders_columns:
            db.execute(text("ALTER TABLE orders ADD COLUMN company_name VARCHAR(180) NULL"))
        if "amount_in_rupee" not in orders_columns:
            db.execute(text("ALTER TABLE orders ADD COLUMN amount_in_rupee DOUBLE NOT NULL DEFAULT 0.0"))
        if "quantity" not in orders_columns:
            db.execute(text("ALTER TABLE orders ADD COLUMN quantity DOUBLE NOT NULL DEFAULT 1.0"))
        db.commit()

    if "boms" not in tables:
        if dialect == "mysql":
            db.execute(
                text(
                    """
    ...
    if "bom_items" not in tables:
        if dialect == "mysql":
            ...
            db.execute(text("CREATE INDEX ix_bom_items_bom_id ON bom_items (bom_id)"))
        db.commit()

    if "bom_items" in tables:
        bom_items_columns = {column["name"] for column in inspector.get_columns("bom_items")}
        if "available_stock" not in bom_items_columns:
            db.execute(text("ALTER TABLE bom_items ADD COLUMN available_stock DOUBLE NOT NULL DEFAULT 0.0"))
        if "supplier" not in bom_items_columns:
            db.execute(text("ALTER TABLE bom_items ADD COLUMN supplier VARCHAR(255) NULL"))
        db.commit()
